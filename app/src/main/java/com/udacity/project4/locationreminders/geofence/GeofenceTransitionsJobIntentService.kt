        ...
        if (intent.action == ACTION_GEOFENCE_EVENT) {
            ...
            if (geofencingEvent.geofenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER) {
                if (geofencingEvent.triggeringGeofences!!.isNotEmpty()) {
                    ...
                    Log.d(TAG, R.string.geofence_entered.toString())
                    sendNotification(triggeringGeofences)
                } else {
                    Log.e(TAG, R.string.geofence_not_available.toString())
                }
            }
        }
    }
...
//        Interaction to the repository has to be through a coroutine scope
        for (t in triggeringGeofences.indices) {
            val requestId = triggeringGeofences[t].requestId
